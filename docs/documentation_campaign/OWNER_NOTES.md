...
| ON-015 | Canonical cross-project docs | Glennergy-ESP owns canonical cross-project documentation | Owner decision, partially superseded by ON-017 | ESP retains system context, glossary and limitations ownership; HTTP documentation now follows ON-017 |
...
| ON-017 | HTTP API documentation ownership | Glennergy contains the complete server HTTP API reference; Glennergy-ESP contains all endpoint and behavior details relevant to the firmware | Owner decision superseding the interface portion of ON-015 | Maintain synchronized server and consumer documents; shared wire changes update both |
