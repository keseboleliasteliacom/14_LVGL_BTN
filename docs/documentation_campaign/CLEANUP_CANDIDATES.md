...
| CC-015 | ESP source comment | Commented credential-like Wi-Fi literal is committed | Independent source audit; value intentionally not copied into campaign docs | Remove in separate approved code cleanup and rotate if it was ever real/used | Security cleanup candidate |
| CC-016 | ESP LEOP configuration | Concrete deployment endpoint is compiled into source | LEOP fetcher source; address intentionally omitted from public docs | Move to an appropriate configuration mechanism if non-disclosure or environment portability is desired | Configuration cleanup candidate |
